...
function checkEnvConfigured() {
  ...
  const hasViteKey = content.includes('VITE_GEMINI_API_KEY=') &&
                     !content.includes('VITE_GEMINI_API_KEY=your_') &&
                     !content.includes('VITE_GEMINI_API_KEY=\n') &&
                     !content.includes('VITE_GEMINI_API_KEY=\r');
  const hasLegacyKey = content.includes('GEMINI_API_KEY=') &&
                       !content.includes('GEMINI_API_KEY=your_') &&
                       !content.includes('GEMINI_API_KEY=\n') &&
                       !content.includes('GEMINI_API_KEY=\r');
  const hasKey = hasViteKey || hasLegacyKey;
...
async function runWizard() {
  ...
  try {
    // Step 1: System Check
    print();
    printColor(c.bright + c.white, '━━━ STEP 1: System Requirements ━━━');
    print();
    ...
    print();
    printColor(c.bright + c.white, '━━━ STEP 2: Install Dependencies ━━━');
    print();
    ...
    print();
    printColor(c.bright + c.white, '━━━ STEP 3: Configure Gemini API Key ━━━');
    print();
    ...
    // Step 4: Completion
    print();
    printColor(c.bright + c.white, '━━━ STEP 4: Setup Complete! ━━━');
    print();

    printBox([
      '                                    ',
      '  🎉 DorkNexus is ready to use!    ',
      '                                    ',
    ], c.green);
    ...
    print();
    printColor(c.cyan, '  npm run dev      ' + c.dim + '→ Start development server');
    printColor(c.cyan, '  npm run build    ' + c.dim + '→ Build for production');
    printColor(c.cyan, '  npm run preview  ' + c.dim + '→ Preview production build');
    print();
